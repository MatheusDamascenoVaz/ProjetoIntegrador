...
public class RelatorioValidadeMySQL

{

    public DatabaseService _databaseService;

    public RelatorioValidadeMySQL(DatabaseService databaseService)
    {
        _databaseService = databaseService;
    }

    public void GerarRelatorio60DiasValidade()
    {
        ...
        SaveFileDialog saveFileDialog = new SaveFileDialog
        {
            ...
            Title = "Salvar Relatório de Validade",
            FileName = $"RelatorioValidade60Dias_{DateTime.Now:yyyyMMdd}.pdf"
        };
        ...
        if (saveFileDialog.ShowDialog() == DialogResult.OK)
        {
            try
            {
                ...
                // Adicionar título
                Paragraph titulo = new Paragraph("RELATÓRIO DE PRODUTOS PRÓXIMOS DA VALIDADE (60 DIAS)",
                ...
                DataTable dt = GetProdutosProximosValidade();

                // Verificar se há dados
                if (dt == null || dt.Rows.Count == 0)
                {
                    Paragraph semDados = new Paragraph("Nenhum produto próximo da validade encontrado.",
                ...
                else
                {
                    ...
                    Paragraph rodape = new Paragraph($"Total de produtos próximos da validade: {dt.Rows.Count}",
    ...
    private DataTable GetProdutosProximosValidade( )
    {
        string query = @"
        SELECT * FROM produtos 
        WHERE validade BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 60 DAY)";
